...
static Arena* arena_create(size_t size) {
    ...
    if (VEX_UNLIKELY(!memory)) return NULL;
...
VEX_INLINE VEX_HOT
void* freelist_alloc(int size_class) {
...
VEX_INLINE VEX_HOT
int freelist_free(void* ptr, int size_class) {
    FreeList* list = &g_free_lists[size_class];
...
void* vex_malloc(size_t size) {
    if (VEX_UNLIKELY(size == 0)) return NULL;
...
void* vex_realloc(void* ptr, size_t size) {
    if (VEX_UNLIKELY(!ptr)) return vex_malloc(size);
...
char* vex_strdup(const char* str) {
    if (VEX_UNLIKELY(!str)) return NULL;
