...
vex_cmd_result_t *vex_cmd_exec(vex_cmd_config_t *config)
{
  ...
  if (config->capture_stdout)
  {
    ...
    result->stdout_data = (char *)vex_malloc(capacity);
                     ...
                     capacity - result->stdout_size)) > 0)
    {
      ...
      {
        capacity *= 2;
        result->stdout_data = (char *)vex_realloc(result->stdout_data, capacity);
      }
    }
  ...
  {
    size_t capacity = 4096;
    result->stderr_data = (char *)vex_malloc(capacity);
                     ...
                     capacity - result->stderr_size)) > 0)
    {
      ...
      {
        capacity *= 2;
        result->stderr_data = (char *)vex_realloc(result->stderr_data, capacity);
      }
    }
