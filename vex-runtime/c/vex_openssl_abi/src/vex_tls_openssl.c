...
static void set_err_from_queue(void){
  ...
  if (e) ERR_error_string_n(e, g_err, sizeof(g_err));
  else snprintf(g_err, sizeof(g_err), "tls error");
}
...
static int parse_alpn_csv(const char* csv, unsigned char** out, unsigned* out_len){
  if (!csv) { *out = NULL; *out_len = 0; return 0; }
  size_t n = strlen(csv);
  unsigned char* buf = (unsigned char*)malloc(n + 1 + 16);
  if (!buf) return -1;
  unsigned pos = 0;
  const char* p = csv;
  while (*p) {
  ...
  *out = buf; *out_len = pos; return 0;
}

VexTlsCtx* vex_tls_ctx_create(const VexTlsConfig* cfg){
  if (!cfg) { set_err("NULL cfg"); return NULL; }
  ...
  if (!c->ctx) { set_err_from_queue(); free(c); return NULL; }
  ...
  if (cfg->is_server && cfg->cert_pem && cfg->key_pem) {
    if (SSL_CTX_use_certificate_chain_file(c->ctx, cfg->cert_pem) != 1) { set_err_from_queue(); goto fail; }
    if (SSL_CTX_use_PrivateKey_file(c->ctx, cfg->key_pem, SSL_FILETYPE_PEM) != 1) { set_err_from_queue(); goto fail; }
    if (SSL_CTX_check_private_key(c->ctx) != 1) { set_err("private key mismatch"); goto fail; }
  }

#ifdef TLSEXT_TYPE_application_layer_protocol_negotiation
  if (cfg->alpn_csv) {
    unsigned char* wire = NULL; unsigned wlen = 0;
    if (parse_alpn_csv(cfg->alpn_csv, &wire, &wlen) != 0) { set_err("alpn parse"); goto fail; }
    if (SSL_CTX_set_alpn_protos(c->ctx, wire, wlen) != 0) { free(wire); set_err_from_queue(); goto fail; }
    free(wire);
  }
#endif
...
VexTlsConn* vex_tls_conn_wrap_fd(VexTlsCtx* ctx, int fd){
  if (!ctx) { set_err("NULL ctx"); return NULL; }
  VexTlsConn* c = (VexTlsConn*)calloc(1, sizeof(*c));
  if (!c) { set_err("oom"); return NULL; }
  c->fd = fd;
  c->ssl = SSL_new(ctx->ctx);
  if (!c->ssl) { set_err_from_queue(); free(c); return NULL; }
  SSL_set_fd(c->ssl, fd);
  if (ctx->is_server) SSL_set_accept_state(c->ssl);
...
void vex_tls_conn_destroy(VexTlsConn* c){
  if (!c) return;
  if (c->alpn_selected) free(c->alpn_selected);
  if (c->ssl) SSL_free(c->ssl);
  free(c);
}

static VexTlsStatus map_err(int r, SSL* s){
  if (r > 0) return VEX_TLS_OK;
  int e = SSL_get_error(s, r);
  if (e == SSL_ERROR_WANT_READ)  return VEX_TLS_WANT_READ;
  if (e == SSL_ERROR_WANT_WRITE) return VEX_TLS_WANT_WRITE;
  set_err_from_queue();
  return VEX_TLS_ERR;
}
...
VexTlsStatus vex_tls_handshake(VexTlsConn* c){
  ...
  if (r > 0) {
    ...
    if (len > 0 && !c->alpn_selected) {
      c->alpn_selected = (char*)malloc(len + 1);
      if (c->alpn_selected) {
...
VexTlsStatus vex_tls_read(VexTlsConn* c, uint8_t* buf, size_t cap, size_t* out_n){
  ...
  if (r > 0) { if (out_n) *out_n = (size_t)r; return VEX_TLS_OK; }
  return map_err(r, c->ssl);
#endif
}
...
int vex_tls_shutdown(VexTlsConn* c){
  ...
  if (r < 0) { set_err_from_queue(); return -1; }
  return 0;
}
