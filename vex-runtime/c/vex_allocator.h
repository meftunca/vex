...
/* Set VEX_ALLOCATOR=mimalloc (default) or system */
    ...
    #define vex_calloc(count, size)      mi_calloc(count, size)
    ...
    #define vex_calloc(count, size)      calloc(count, size)
 ...
 */
#define VEX_ALLOC_TYPE(type) ((type*)vex_alloc(sizeof(type)))

/**
 * Allocate array of specific type
 * Usage: int* arr = VEX_ALLOC_ARRAY(int, 100);
 */
#define VEX_ALLOC_ARRAY(type, count) ((type*)vex_calloc(count, sizeof(type)))

/**
 * Reallocate array with new size
 * Usage: arr = VEX_REALLOC_ARRAY(arr, int, new_count);
 */
#define VEX_REALLOC_ARRAY(ptr, type, count) ((type*)vex_realloc(ptr, (count) * sizeof(type)))
