...
VexTimeSched *vt_sched_create_uring(void)
{
  ...
  if (!s)
    return NULL;
  if (io_uring_queue_init(128, &s->ring, 0) != 0)
  {
    VEX_FREE_IMPL(s);
    return NULL;
  }
  s->running = 1;
  if (pthread_create(&s->th, NULL, ur_worker, s) != 0)
  {
...
int vt_timer_start(VexTimer *t, VexDuration after_ns)
{
  ...
  t->n = (Node *)VEX_CALLOC_IMPL(1, sizeof(Node));
  if (!t->n)
    return -1;
...
VexTicker *vt_ticker_create(VexTimeSched *s, VexTimeCb cb, void *user)
{
  ...
  tk->cb = cb;
  tk->user = user;
  tk->n = (Node *)VEX_CALLOC_IMPL(1, sizeof(Node));
  if (!tk->n)
  {
    VEX_FREE_IMPL(tk);
    return NULL;
  }
  tk->n->key = (uint64_t)(uintptr_t)tk->n;
  tk->n->periodic = 1;
  tk->n->period_ns = 0;
  tk->n->cb = cb;
  tk->n->user = user;
