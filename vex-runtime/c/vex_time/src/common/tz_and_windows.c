...
static int64_t read_be64(const unsigned char *p)
{
  int64_t v = 0;
  for (int i = 0; i < 8; i++)
  {
...
static VexTz *tz_utc_singleton(void)
{
  ...
  if (!inited)
  {
    utc.is_fixed = 1;
    utc.fixed_offset = 0;
    snprintf(utc.fixed_name, sizeof(utc.fixed_name), "UTC");
    inited = 1;
  }
  return &utc;
}
...
{
  VexTz *z = (VexTz *)VEX_CALLOC_IMPL(1, sizeof(VexTz));
  z->is_fixed = 1;
  z->fixed_offset = offset_sec;
...
static int readn(ReadContext *ctx, void *dst, size_t n)
{
  if (ctx->f)
  {
    return fread(dst, 1, n, ctx->f) == n ? 0 : -1;
  }
  if (ctx->off + n > ctx->len)
    return -1;
...
static VexTz *load_tzif_stream(FILE *f, const unsigned char *mem, size_t len)
{
  ...
  VexTz *z = (VexTz *)VEX_CALLOC_IMPL(1, sizeof(VexTz));
  z->has_tzif = 1;
  z->timecnt = tzh_timecnt;
  z->typecnt = tzh_typecnt;
  z->charcnt = tzh_charcnt;
  z->trans = (int64_t *)VEX_CALLOC_IMPL(tzh_timecnt, sizeof(int64_t));
  z->trans_type = (unsigned char *)VEX_CALLOC_IMPL(tzh_timecnt, 1);
  z->ttis = (struct Ttinfo *)VEX_CALLOC_IMPL(tzh_typecnt, sizeof(struct Ttinfo));
  z->abbrs = (char *)VEX_CALLOC_IMPL(tzh_charcnt + 1, 1);

  for (int i = 0; i < tzh_timecnt; i++)
  {
    unsigned char buf[8];
    if (readn(&ctx, buf, 8) != 0)
    {
      vt_tz_release(z);
      return NULL;
    }
    z->trans[i] = read_be64(buf);
  }
  if (readn(&ctx, z->trans_type, tzh_timecnt) != 0)
  {
    vt_tz_release(z);
    return NULL;
  }
  ...
  if (readn(&ctx, z->abbrs, tzh_charcnt) != 0)
  {
    vt_tz_release(z);
    return NULL;
  }
...
void vt_tz_release(VexTz *tz)
{
  ...
  {
    VEX_FREE_IMPL(tz->trans);
    VEX_FREE_IMPL(tz->trans_type);
    VEX_FREE_IMPL(tz->ttis);
    VEX_FREE_IMPL(tz->abbrs);
    if (tz->mem_blob)
      VEX_FREE_IMPL(tz->mem_blob);
  }
  VEX_FREE_IMPL(tz);
}
...
{
  int off = 0;
  vt_tz_offset_at(tz, utc, &off, NULL);
...
static const char *map_win_to_iana(const char *win)
{
  ...
  {
    if (_stricmp(WIN_TO_IANA[i].win, win) == 0)
...
VexTz *vt_tz_local(void)
{
  ...
  char keyname[256];
  WideCharToMultiByte(CP_UTF8, 0, dtzi.TimeZoneKeyName, -1, keyname, sizeof keyname, NULL, NULL);
  if (keyname[0])
    iana = map_win_to_iana(keyname);
  if (!iana)
  {
    char stdname[256];
    WideCharToMultiByte(CP_UTF8, 0, dtzi.StandardName, -1, stdname, sizeof stdname, NULL, NULL);
    if (stdname[0])
      iana = map_win_to_iana(stdname);
  }
  if (iana)
  {
