...
static int append_int_no_trailing_zeros(char** buf, size_t* remain, int val, int max_digits) {
    ...
    /* First, pad to max_digits with leading zeros */
    char temp[16];
    int tmp = val;
    ...
    int end_pos = max_digits - 1;
    while (end_pos > 0 && temp[end_pos] == '0') {
        end_pos--;
    }
    
    /* Copy to buffer */
    int len = end_pos + 1;
    if ((size_t)len + 1 > *remain) return -1;
...
int vt_format_layout(VexTime t, const char* layout, char* buf, size_t buflen) {
    ...
    while (*l && remain > 1) {
        ...
        if (l[0] == '2') {
            ...
            if (l[2] == '9' && l[3] == '9' && l[4] == '9' && l[5] == '9' && l[6] == '9' && l[7] == '9' && l[8] == '9' && l[9] == '9') {
