...
static VexConnection redis_connect(const char *conninfo)
{
    ...
    if (conninfo && strlen(conninfo) > 0)
    {
        ...
        if (conninfo[0] == '/')
        {
            RedisContext *rctx = (RedisContext *)vex_calloc(1, sizeof(RedisContext));
            if (!rctx)
            {
...
static void redis_clear_result(VexResultSet *res)
{
    ...
    if (rres->reply)
        freeReplyObject(rres->reply);
    if (rres->current_str)
        vex_free(rres->current_str);
    free(rres);
                                        ...
                                        int nParams, const VexDbValue *params)
{
    ...
    if (nParams > 0)
    {
        ...
        const char **argv = (const char **)vex_malloc((nParams + 1) * sizeof(char *));
        size_t *argvlen = (size_t *)vex_malloc((nParams + 1) * sizeof(size_t));
    ...
    else
    {
        ...
        // Allocate argv
        const char **argv = (const char **)vex_malloc(word_count * sizeof(char *));
        size_t *argvlen = (size_t *)vex_malloc(word_count * sizeof(size_t));
...
static VexDbPayload redis_fetch_next(VexResultSet *res)
{
    ...
    if (reply->type == REDIS_REPLY_ARRAY)
    {
        ...
        case REDIS_REPLY_INTEGER:
            ...
            rres->current_str = vex_malloc(32);
            if (rres->current_str)
            {
...
{
    if (!conn || !conn->native_conn || !channel)
        return -1;
...
{
    if (!conn || !conn->native_conn || !channel || !message)
        return -1;
