...
static VexConnection mongo_connect(const char *conninfo) {
    ...
    MongoContext *ctx = (MongoContext*)calloc(1, sizeof(MongoContext));
    if (!ctx) {
...
static void mongo_disconnect(VexConnection *conn) {
    ...
    if (ctx->database) mongoc_database_destroy(ctx->database);
    if (ctx->client) mongoc_client_destroy(ctx->client);
    free(ctx);
...
static void mongo_clear_result(VexResultSet *res) {
    ...
    if (mres->cursor) mongoc_cursor_destroy(mres->cursor);
    if (mres->json_str) free(mres->json_str);
    free(mres);
...
static VexResultSet mongo_execute_query(VexConnection *conn, const char *query,
    ...
    if (coll_len >= sizeof(collection_name)) coll_len = sizeof(collection_name) - 1;
    ...
    if (args_len >= sizeof(args)) args_len = sizeof(args) - 1;
    ...
    MongoResultContext *mres = (MongoResultContext*)calloc(1, sizeof(MongoResultContext));
    if (!mres) {
