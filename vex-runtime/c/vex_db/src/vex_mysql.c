...
static void parse_conninfo(const char *s, ConnInfo *ci)
{
    ...
    if (!s)
        return;
    char *dup = vex_strdup(s);
    for (char *tok = strtok(dup, " ;"); tok; tok = strtok(NULL, " ;"))
    {
        if (strncmp(tok, "host=", 5) == 0)
            ci->host = tok + 5;
        else if (strncmp(tok, "user=", 5) == 0)
            ci->user = tok + 5;
        else if (strncmp(tok, "password=", 9) == 0)
            ci->pass = tok + 9;
        else if (strncmp(tok, "db=", 3) == 0)
            ci->db = tok + 3;
        else if (strncmp(tok, "port=", 5) == 0)
            ci->port = (unsigned)atoi(tok + 5);
    }
...
static VexConnection my_connect(const char *conninfo)
{
    ...
    if (!m)
    {
        ...
        snprintf(c.error.message, sizeof(c.error.message), "mysql_init failed");
        return c;
    }
    ConnInfo ci;
    parse_conninfo(conninfo, &ci);
...
{
    (void)n;
    (void)p; /* Simple text query. For prepared/binary, extend here. */
    VexResultSet rs = {0};
