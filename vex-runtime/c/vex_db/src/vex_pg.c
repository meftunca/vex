...
static VexConnection pg_connect(const char *conninfo)
{
    ...
    PgContext *ctx = (PgContext *)vex_malloc(sizeof(PgContext));
    if (!ctx)
    {
                                     ...
                                     int nParams, const VexDbValue *params)
{
    ...
    {
        vals[i] = (const char *)params[i].data;
        lens[i] = (int)params[i].length;
        fmts[i] = params[i].is_binary ? 1 : 0;
    }
...
static int pg_wants_read(VexConnection *c)
{
    (void)c;
    return 1;
}
static int pg_wants_write(VexConnection *c)
{
    (void)c;
    return 0;
}
static int pg_start_execute(VexConnection *c, const char *q, int n, const VexDbValue *p)
{
    ...
    for (int i = 0; i < n; i++)
    {
        vals[i] = (const char *)p[i].data;
        lens[i] = (int)p[i].length;
        fmts[i] = p[i].is_binary ? 1 : 0;
    }
...
{
    VexResultSet rs = {0};
    PGresult *pr = PQgetResult(((PgContext *)c->native_conn)->conn);
    if (!pr)
    {
        rs.error.code = VEX_DB_ERROR_NOT_FOUND;
        return rs;
    }
    rs.native_result = pr;
    rs.column_count = (uint32_t)PQnfields(pr);
