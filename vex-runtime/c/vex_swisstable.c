...
static inline uint32_t simd_group_match_eq_neon(const uint8_t* p, uint8_t byte) {
    ...
    vst1q_u8(tmp, eq);
    uint32_t m = 0;
    for (int i = 0; i < 16; ++i) m |= (uint32_t)(tmp[i] >> 7) << i;
    return m;
#else
    (void)p; (void)byte;
    return 0;
...
static inline uint32_t simd_group_match_eq_scalar(const uint8_t* p, uint8_t byte) {
    uint32_t m = 0;
    for (int i = 0; i < GROUP_SIZE; ++i) m |= (uint32_t)(p[i] == byte) << i;
    return m;
}
...
#if defined(_MSC_VER)
    unsigned long idx;
    if (_BitScanForward(&idx, mask)) return (int)idx;
    return -1;
#else
    if (!mask) return -1;
...
static bool vex_swiss_rehash(SwissMap *map, size_t new_cap) {
    SwissMap nm;
    if (!vex_swiss_init_internal(&nm, new_cap)) return false;
...
static bool vex_swiss_init_internal(SwissMap *map, size_t initial_capacity) {
    ...
    if (!map->entries || !map->ctrl) return false;
...
static bool vex_swiss_insert_internal(SwissMap *map, const char *key, void *value) {
    if ((map->len + 1) * 2 > map->capacity) {
        if (!vex_swiss_rehash(map, map->capacity * 2)) return false;
    }
...
static void *vex_swiss_get_internal(const SwissMap *map, const char *key) {
    ...
    for (;;) {
        ...
        i += GROUP_SIZE;
        scanned += GROUP_SIZE;
        if (scanned > cap) return NULL; // safety
    }
}
