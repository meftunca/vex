...
static CoroStatus producer_coro(WorkerContext *ctx, void *data)
{
    ...
    case 1:
    { // After timer, go back to enqueue
        ps->state = 0;
...
static CoroStatus consumer_coro(WorkerContext *ctx, void *data)
{
    ...
    case 1:
    { // After processing, go back to dequeue
        cs->state = 0;
        return CORO_STATUS_RUNNING;
    }
    }
...
int main(void)
{
    ...
    {
        ProducerState *ps = (ProducerState *)xmalloc(sizeof(ProducerState));
        ps->sh = sh;
        ps->my_id = i;
        ps->i = 0;
        ps->state = 0;
    ...
    {
        ConsumerState *cs = (ConsumerState *)xmalloc(sizeof(ConsumerState));
        cs->sh = sh;
        cs->state = 0;
    ...
    SupervisorState *ss = (SupervisorState *)xmalloc(sizeof(SupervisorState));
    ss->sh = sh;
    ss->tick = 0;
