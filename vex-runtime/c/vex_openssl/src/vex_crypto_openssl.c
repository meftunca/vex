...
static void set_err_from_queue(void)
{
  ...
  if (e)
    ERR_error_string_n(e, g_cerr, sizeof(g_cerr));
  else
    snprintf(g_cerr, sizeof(g_cerr), "crypto error");
}
...
static const EVP_CIPHER *aead_from_name(const char *n, int *is_aead_mode)
{
  *is_aead_mode = 1;
  if (!n)
    return NULL;
  if (strcmp(n, "AES-128-GCM") == 0)
    return EVP_aes_128_gcm();
  if (strcmp(n, "AES-256-GCM") == 0)
    return EVP_aes_256_gcm();
  if (strcmp(n, "CHACHA20-POLY1305") == 0)
                  ...
                  size_t tag_len)
{
  ...
  if (EVP_EncryptInit_ex(ctx, ciph, NULL, NULL, NULL) != 1)
    goto done;
  if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, (int)nonce_len, NULL) != 1)
    goto done;
  if (EVP_EncryptInit_ex(ctx, NULL, NULL, key, nonce) != 1)
    goto done;
  if (ad && ad_len)
  {
    if (EVP_EncryptUpdate(ctx, NULL, &outl, ad, (int)ad_len) != 1)
      goto done;
  }
  if (pt_len)
  {
    if (EVP_EncryptUpdate(ctx, out_cipher, &outl, plaintext, (int)pt_len) != 1)
      goto done;
    len = outl;
  }
  if (EVP_EncryptFinal_ex(ctx, out_cipher + len, &outl) != 1)
    goto done;
  len += outl;
  if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, (int)tag_len, out_cipher + len) != 1)
    goto done;
  len += (int)tag_len;
                  ...
                  size_t tag_len)
{
  ...
  int ok = 0, outl = 0, len = 0;
  if (EVP_DecryptInit_ex(ctx, ciph, NULL, NULL, NULL) != 1)
    goto done;
  if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, (int)nonce_len, NULL) != 1)
    goto done;
  ...
  if (ad && ad_len)
  {
    if (EVP_DecryptUpdate(ctx, NULL, &outl, ad, (int)ad_len) != 1)
      goto done;
  }
  if (data_len)
  {
    if (EVP_DecryptUpdate(ctx, out_plain, &outl, ciphertext, (int)data_len) != 1)
      goto done;
    len = outl;
  }
  if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, (int)tag_len, (void *)tag) != 1)
    goto done;
  if (EVP_DecryptFinal_ex(ctx, out_plain + len, &outl) != 1)
  {
    set_err("tag mismatch");
    goto done;
  }
  len += outl;
  *inout_pt_len = (size_t)len;
  ...
  if (!ok && strcmp(g_cerr, "tag mismatch") != 0)
    set_err_from_queue();
  EVP_CIPHER_CTX_free(ctx);
  return ok ? 0 : -1;
}
...
static const EVP_MD *md_from_name(const char *n)
{
  if (!n)
    return NULL;
  if (strcmp(n, "SHA-256") == 0)
    return EVP_sha256();
  if (strcmp(n, "SHA-512") == 0)
    return EVP_sha512();
  if (strcmp(n, "SHA3-256") == 0)
    return EVP_sha3_256();
  if (strcmp(n, "SHA3-512") == 0)
    return EVP_sha3_512();
  return NULL;
}
...
int vex_hash(const char *algo, const uint8_t *msg, size_t len, uint8_t *out_digest, size_t *inout_len)
{
  ...
  if (EVP_DigestUpdate(ctx, msg, len) != 1)
    goto done;
  unsigned outl = 0;
  if (EVP_DigestFinal_ex(ctx, out_digest, &outl) != 1)
    goto done;
  if (inout_len)
    *inout_len = outl;
  ok = 1;
             ...
             uint8_t *out_okm, size_t okm_len)
{
  ...
  if (strcmp(algo, "HKDF-SHA256") == 0)
    md = EVP_sha256();
  else if (strcmp(algo, "HKDF-SHA512") == 0)
    md = EVP_sha512();
  else
  {
    set_err("unsupported hkdf");
    return -1;
  }
  ...
  if (EVP_PKEY_derive(pctx, out_okm, &outlen) <= 0)
    goto done;
  ok = 1;
done:
  if (!ok)
...
int vex_x25519(uint8_t shared[32], const uint8_t priv[32], const uint8_t peer_pub[32])
{
  ...
  if (EVP_PKEY_derive(ctx, shared, &slen) != 1 || slen != 32)
  {
    set_err_from_queue();
    goto done3;
  }
...
int vex_ed25519_sign(uint8_t sig[64], const uint8_t *msg, size_t len, const uint8_t sk[64])
{
  ...
  if (EVP_DigestSign(ctx, sig, &slen, msg, len) != 1 || slen != 64)
  {
    set_err_from_queue();
    goto done;
  }
...
static const EVP_CIPHER *cipher_from_name(const char *n)
{
  if (!n)
    return NULL;
  if (strcmp(n, "AES-128-CBC") == 0)
    return EVP_aes_128_cbc();
  if (strcmp(n, "AES-256-CBC") == 0)
    return EVP_aes_256_cbc();
  if (strcmp(n, "AES-128-CTR") == 0)
    return EVP_aes_128_ctr();
  if (strcmp(n, "AES-256-CTR") == 0)
                       ...
                       uint8_t *out_cipher, size_t *out_ct_len)
{
  ...
  {
    set_err("invalid key length");
    return -1;
  }
  if ((int)iv_len != expected_iv_len)
  {
    set_err("invalid iv length");
    return -1;
  }
                       ...
                       uint8_t *out_plain, size_t *out_pt_len)
{
  ...
  if (ct_len > 0)
  {
    if (EVP_DecryptUpdate(ctx, out_plain, &outl, ciphertext, (int)ct_len) != 1)
      goto done;
    len = outl;
  }
  if (EVP_DecryptFinal_ex(ctx, out_plain + len, &outl) != 1)
             ...
             uint8_t *out_mac, size_t *out_mac_len)
{
  ...
  if (!md)
  {
    set_err("unsupported hash for hmac");
    return -1;
  }
               ...
               uint8_t *out_key, size_t key_len)
{
  ...
  if (!md)
  {
    set_err("unsupported hash for pbkdf2");
    return -1;
  }
                             ...
                             uint8_t **out_private_der, size_t *priv_len)
{
  ...
  *out_public_der = (uint8_t *)vex_malloc((size_t)pub_der_len);
  if (!*out_public_der)
  {
    set_err("oom");
    EVP_PKEY_free(pkey);
  ...
  *pub_len = (size_t)pub_der_len;
  ...
  {
    set_err_from_queue();
    vex_free(*out_public_der);
    EVP_PKEY_free(pkey);
    return -1;
  }
  *out_private_der = (uint8_t *)vex_malloc((size_t)priv_der_len);
  if (!*out_private_der)
  {
    set_err("oom");
    vex_free(*out_public_der);
                 ...
                 uint8_t *out_sig, size_t *sig_len)
{
  ...
  if (EVP_DigestSign(ctx, out_sig, &slen, msg, msg_len) != 1)
  {
    set_err_from_queue();
    goto done;
  }
                    ...
                    uint8_t *out_cipher, size_t *ct_len)
{
  ...
  if (EVP_PKEY_encrypt(ctx, out_cipher, &outlen, plaintext, pt_len) <= 0)
  {
    set_err_from_queue();
    goto done;
  }
                    ...
                    uint8_t *out_plain, size_t *pt_len)
{
  ...
  if (EVP_PKEY_decrypt(ctx, out_plain, &outlen, ciphertext, ct_len) <= 0)
  {
    set_err_from_queue();
    goto done;
  }
             ...
             uint8_t *out_shared, size_t *shared_len)
{
  ...
  if (EVP_PKEY_derive(ctx, out_shared, &slen) != 1)
  {
    set_err_from_queue();
    goto done;
  }
...
{
  if (!subject || !out_cert_pem || !out_key_pem)
  {
    set_err("NULL args");
    return -1;
  }
  ...
  *out_cert_pem = vex_malloc(cert_mem->length);
  ...
  *out_key_pem = vex_malloc(key_mem->length);
...
{
  if (!subject || !private_key_pem || !out_csr_pem)
  {
    set_err("NULL args");
    return -1;
  }
  ...
  *out_csr_pem = vex_malloc(csr_mem->length);
...
{
  if (!msg || !out_digest || !out_len)
  {
    set_err("NULL args");
    return -1;
  }
  if (*out_len < MD5_DIGEST_LENGTH)
  {
...
int vex_sha384(const uint8_t *msg, size_t len, uint8_t *out_digest, size_t *out_len)
{
  ...
  if (*out_len < SHA384_DIGEST_LENGTH)
  {
    set_err("buffer too small");
    return -1;
  }
...
{
  if (!kek || !plaintext_key || !out_wrapped || !out_len)
  {
    set_err("NULL args");
    return -1;
  }
  ...
  if (pt_len % 8 != 0)
  {
    set_err("plaintext key must be multiple of 8 bytes");
    return -1;
  }
...
{
  if (!kek || !wrapped_key || !out_plaintext || !out_len)
  {
    set_err("NULL args");
    return -1;
  }
