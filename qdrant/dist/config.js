...
export const collectionNames = ['vex_docs', 'vex_code', 'vex_todos', 'vex_examples', 'vex_tests'];
export const config = {
    qdrant: {
    ...
    paths: {
        ...
        code: (process.env.CODE_DIRS || '../vex-libs,../vex-compiler,../vex-parser,../vex-runtime')
            .split(',')
            .map(d => resolve(__dirname, d.trim())),
        examples: resolve(__dirname, process.env.EXAMPLES_DIR || '../examples'),
        todoFile: resolve(__dirname, process.env.TODO_FILE || '../.github/copilot-instructions.md'),
    },
    chunking: {
