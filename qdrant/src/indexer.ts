...
export class DocumentationIndexer {
   ...
   */
  async indexSpecifications(): Promise<void> {
    ...
    console.log(
      `✓ Indexed ${mdFiles.length} specification files (${totalChunks} chunks)`
    );
  }
  ...
  async indexDocs(): Promise<void> {
    ...
    console.log(
      `✓ Indexed ${mdFiles.length} documentation files (${totalChunks} chunks)`
    );
  }
   ...
   * Index examples
   */
  async indexExamples(): Promise<void> {
    console.log("\n📦 Indexing Examples...");
    ...
    console.log(
      `✓ Indexed ${vxFiles.length} example files (${totalChunks} chunks)`
    );
  }
  ...
  async indexTodos(): Promise<void> {
    ...
    for (let i = 0; i < todos.length; i++) {
      const todo = todos[i];
      const embedding = await this.client.generateEmbedding(
        `${todo.title}\n${todo.description}`
      );
   ...
   * Index a code file
   */
  private async indexCodeFile(filePath: string): Promise<number> {
    const content = readFileSync(filePath, "utf-8");
    const relativePath = relative(config.paths.root, filePath);
   ...
   */
  private async indexExampleFile(filePath: string): Promise<number> {
    const content = readFileSync(filePath, "utf-8");
   ...
   */
  private parseTodos(content: string): Array<{
    title: string;
    description: string;
    status: string;
    files: string[];
