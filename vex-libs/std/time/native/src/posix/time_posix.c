...
static void* worker(void* arg){
  ...
  while (s->running){
    ...
    if (due>now){
      struct timespec ts; ts.tv_sec=(due-now)/1000000000LL; ts.tv_nsec=(due-now)%1000000000LL;
...
VexTimeSched* vt_sched_create(void){
  struct VexTimeSched* s = (struct VexTimeSched*)calloc(1,sizeof(*s));
  if (!s) return NULL;
  pthread_mutex_init(&s->mu,NULL);
  pthread_cond_init(&s->cv,NULL);
  s->running=1;
  if (pthread_create(&s->th, NULL, worker, s)!=0){ s->running=0; free(s); return NULL; }
  return s;
}
...
struct VexTimer  { struct VexTimeSched* s; HeapNode node; };
struct VexTicker { struct VexTimeSched* s; HeapNode node; };
...
VexTimer* vt_timer_create(VexTimeSched* s, VexTimeCb cb, void* user){
  ...
  t->s=s; t->node.cb=cb; t->node.user=user; t->node.periodic=0; t->node.active=0; t->node.owner=t;
  return t;
}
int vt_timer_start(VexTimer* t, VexDuration after_ns){
  if (!t) return -1;
  t->node.active=1; t->node.periodic=0; t->node.period_ns=0; t->node.owner=t;
...
VexTicker* vt_ticker_create(VexTimeSched* s, VexTimeCb cb, void* user){
  ...
  tk->s=s; tk->node.cb=cb; tk->node.user=user; tk->node.periodic=1; tk->node.active=0; tk->node.owner=tk;
  return tk;
}
int vt_ticker_start(VexTicker* tk, VexDuration period_ns){
  if (!tk || period_ns<=0) return -1;
  tk->node.active=1; tk->node.periodic=1; tk->node.period_ns=period_ns; tk->node.owner=tk;
