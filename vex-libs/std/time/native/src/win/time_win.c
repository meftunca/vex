...
int vt_sleep_ns(VexDuration ns){
  ...
  if (!h) return -1;
  LARGE_INTEGER li; LONGLONG rel = -(LONGLONG)(ns/100); if (rel==0) rel=-1;
  li.QuadPart = rel;
...
static int heap_remove_owner(VexTimeSched* s, void* owner){
...
int vt_timer_reset(VexTimer* t, VexDuration after_ns){
  if (!t) return -1;
  EnterCriticalSection(&t->s->mu);
  heap_remove_owner(t->s, t);
  LeaveCriticalSection(&t->s->mu);
  t->node.active=1; t->node.periodic=0; t->node.period_ns=0; t->node.owner=t;
  t->node.due_ns = (LONGLONG)vt_monotonic_now_ns() + (after_ns<0?0:after_ns);
  return sched_insert(t->s, t->node);
}
...
int vt_ticker_reset(VexTicker* tk, VexDuration period_ns){
  if (!tk || period_ns<=0) return -1;
  EnterCriticalSection(&tk->s->mu);
  heap_remove_owner(tk->s, tk);
  LeaveCriticalSection(&tk->s->mu);
  tk->node.active=1; tk->node.periodic=1; tk->node.period_ns=period_ns; tk->node.owner=tk;
  tk->node.due_ns = (LONGLONG)vt_monotonic_now_ns() + period_ns;
  return sched_insert(tk->s, tk->node);
}
