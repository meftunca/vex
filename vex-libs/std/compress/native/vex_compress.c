...
vex_buffer_t *vex_gzip_compress(const uint8_t *input, size_t input_size, int level)
{
  ...
  vex_buffer_t *output = (vex_buffer_t *)vex_malloc(sizeof(vex_buffer_t));
  output->data = (uint8_t *)vex_malloc(max_size);
...
vex_buffer_t *vex_gzip_decompress(const uint8_t *input, size_t input_size)
{
  ...
  vex_buffer_t *output = (vex_buffer_t *)vex_malloc(sizeof(vex_buffer_t));
  output->data = (uint8_t *)vex_malloc(output_capacity);
  ...
  while ((ret = inflate(&stream, Z_NO_FLUSH)) == Z_OK)
  {
    ...
    output_capacity *= 2;
    output->data = (uint8_t *)vex_realloc(output->data, output_capacity);
...
vex_buffer_t *vex_lz4_decompress(const uint8_t *input, size_t input_size, size_t decompressed_size)
{
  ...
  vex_buffer_t *output = (vex_buffer_t *)vex_malloc(sizeof(vex_buffer_t));
  output->data = (uint8_t *)vex_malloc(decompressed_size);
                           ...
                           size_t input_size, int level)
{
  ...
  {
  case VEX_COMP_GZIP:
    return vex_gzip_compress(input, input_size, level);
  case VEX_COMP_ZLIB:
    return vex_gzip_compress(input, input_size, level); // Same as gzip
  case VEX_COMP_BZIP2:
    return vex_bzip2_compress(input, input_size, level);
  case VEX_COMP_LZ4:
    return vex_lz4_compress(input, input_size, level);
  case VEX_COMP_ZSTD:
    return vex_zstd_compress(input, input_size, level);
  case VEX_COMP_BROTLI:
    return vex_brotli_compress(input, input_size, level);
  default:
    return NULL;
  }
}
                             ...
                             size_t input_size)
{
  ...
  case VEX_COMP_GZIP:
  case VEX_COMP_ZLIB:
    return vex_gzip_decompress(input, input_size);
  case VEX_COMP_BZIP2:
    return vex_bzip2_decompress(input, input_size);
  case VEX_COMP_LZ4:
    return NULL; // Requires original size
  case VEX_COMP_ZSTD:
    return vex_zstd_decompress(input, input_size);
  case VEX_COMP_BROTLI:
    return vex_brotli_decompress(input, input_size);
  default:
    return NULL;
  }
}
...
vex_buffer_t *vex_lz4_frame_compress(const uint8_t *input, size_t input_size, int level)
{
  ...
  output->data = (uint8_t *)vex_malloc(max_size);ze);
  if (!output->data)
  {
    vex_free(output);
    return NULL;
...
vex_buffer_t *vex_lz4_frame_decompress(const uint8_t *input, size_t input_size)
{
  ...
  {
    output->data = (uint8_t *)vex_realloc(output->data, output->size);
                                         ...
                                         size_t num_samples, size_t dict_size)
{
  ...
  uint8_t *all_samples = (uint8_t *)vex_malloc(total_size);
  if (!all_samples)
    return NULL;
  ...
  // Train dictionary
  vex_compress_dict_t *dict = (vex_compress_dict_t *)vex_malloc(sizeof(vex_compress_dict_t));
  if (!dict)
  {
    vex_free(all_samples);
    return NULL;
  }

  dict->data = (uint8_t *)vex_malloc(dict_size);
  if (!dict->data)
  {
    vex_free(all_samples);
    vex_free(dict);
  ...
  {
    dict->data = (uint8_t *)vex_realloc(dict->data, trained_size);
  }
                                                      ...
                                                      bool finish)
{
  ...
  if (!result)
    return VEX_STREAM_ERROR;
  if (finish && BrotliEncoderIsFinished(state))
    return VEX_STREAM_END;
  return VEX_STREAM_OK;
}
...
int main(void)
{
  ...
  {
    if (fmt == VEX_COMP_LZ4)
      continue; // Skip LZ4 (needs original size for decompression)
