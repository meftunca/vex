...
vex_regex_t *vex_regex_compile(const char *pattern, uint32_t options)
{
  ...
  vex_regex_t *re = (vex_regex_t *)vex_malloc(sizeof(vex_regex_t));
  if (!re)
  {
    return NULL;
  }
...
bool vex_regex_match(vex_regex_t *re, const char *subject, size_t subject_len, vex_match_result_t *result)
{
  ...
  result->full_match.text = (char *)vex_malloc(result->full_match.text_len + 1);
  ...
  {
    result->groups = (vex_match_t *)vex_malloc(sizeof(vex_match_t) * result->group_count);
    for (size_t i = 0; i < result->group_count; i++)
    {
      ...
      else
      {
        ...
        result->groups[i].text = (char *)vex_malloc(result->groups[i].text_len + 1);
...
void vex_match_result_free(vex_match_result_t *result)
{
  ...
  {
    vex_free(result->full_match.text);
  }
  ...
  if (result->groups)
  {
    for (size_t i = 0; i < result->group_count; i++)
    {
      ...
      {
        vex_free(result->groups[i].text);
      }
    }
    vex_free(result->groups);
  }
}
...
vex_match_all_result_t *vex_regex_match_all(vex_regex_t *re, const char *subject, size_t subject_len)
{
  ...
  vex_match_all_result_t *result = (vex_match_all_result_t *)vex_malloc(sizeof(vex_match_all_result_t));
  result->count = 0;
  result->capacity = 8;
  result->matches = (vex_match_result_t *)vex_malloc(sizeof(vex_match_result_t) * result->capacity);
  ...
  while (offset < subject_len)
  {
    ...
    {
      result->capacity *= 2;
      result->matches = (vex_match_result_t *)vex_realloc(result->matches,
    ...
    match->full_match.text = (char *)vex_malloc(match->full_match.text_len + 1);
    ...
    {
      match->groups = (vex_match_t *)vex_malloc(sizeof(vex_match_t) * match->group_count);
      for (size_t i = 0; i < match->group_count; i++)
      {
        ...
        if (match->groups[i].start != PCRE2_UNSET)
        {
          ...
          match->groups[i].text = (char *)vex_malloc(match->groups[i].text_len + 1);
                        ...
                        const char *replacement, size_t *out_len)
{
  ...
  if (!vex_regex_match(re, subject, subject_len, &match))
  {
    ...
    char *result = (char *)vex_malloc(subject_len + 1);
  ...
  char *result = (char *)vex_malloc(new_len + 1);
