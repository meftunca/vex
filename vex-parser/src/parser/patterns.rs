...
impl<'a> Parser<'a> {
    ...
    fn parse_single_pattern(&mut self) -> Result<Pattern, ParseError> {
        ...
        if let Token::Ident(name) = self.peek() {
            ...
            // Check for enum pattern with dot: Result.Ok(x) or Option.None or IpAddr.V4(a,b,c,d)
            if self.match_token(&Token::Dot) {
                let variant = self.consume_identifier()?;

                // Check for data: Variant() or Variant(pattern) or Variant(p1, p2, ...)
            ...
            // Check for old-style enum variant (backward compat): Ok(x), Some(value), V4(a,b,c,d)
            if self.check(&Token::LParen) {
                self.advance();
