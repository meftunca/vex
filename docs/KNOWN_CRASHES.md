...
### 1. println() Post-Execution Bus Error (EXC_BAD_ACCESS) ✅ FIXED

**Status:** RESOLVED  
**Severity:** HIGH  
**Layer:** Layer 0 (Runtime)  
**Discovered:** 15 Kasım 2025  
**Fixed:** 15 Kasım 2025

**Description:**
Programs using `println()` were crashing with bus error after successful execution due to VexValue struct layout mismatch between LLVM IR and C runtime.

**Root Cause:**
VexValue struct alignment mismatch:

- LLVM IR: 32 bytes (i32 type + i8[12] padding + i128 union)
- C runtime (before fix): unaligned union causing memory corruption
- Stack-allocated VexValue arrays had misaligned data access

**Solution:**

1. Added explicit 12-byte padding to VexValue struct in `vex.h`:

   ```c
   typedef struct {
       VexValueType type;        // 4 bytes at offset 0
       uint8_t _padding[12];     // 12 bytes padding to offset 16
       union { __int128 as_i128; ... }; // 16 bytes at offset 16
   } VexValue;
   ```

2. Fixed LLVM VexValue layout in `print_execution.rs`:
   ```rust
   let vex_value_type = codegen.context.struct_type(&[
       codegen.context.i32_type().into(),                    // type
       codegen.context.i8_type().array_type(12).into(),      // padding
       codegen.context.custom_width_int_type(128).into(),    // union
   ], false);
   ```

**Test Results:**

- Single-arg println: ✅ Exit code 0
- Multi-arg println: ✅ Exit code 0, correct output
- Integration tests: 415/421 passing (98.6%, +1 improvement)

**Related Code:**

- `vex-runtime/c/vex.h` - VexValue struct definition
- `vex-compiler/src/codegen_ast/builtins/core/print_execution.rs` - LLVM IR generation
